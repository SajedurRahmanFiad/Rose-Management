      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {role === UserRole.ADMIN && (
          <button 
            ...
            className="flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-3 rounded-xl font-semibold transition-all shadow-lg text-sm"
          >
            <Plus size={18} />
            Add Product
          </button>
        ...
        <div className="p-4 border-b border-gray-100">
          <div className="relative w-full">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2 bg-gray-50 border-transparent focus:bg-white rounded-lg outline-none text-sm"
              value={searchTerm}
          ...
          <table className="w-full text-left">
            <thead className="bg-gray-50 text-gray-500 text-[10px] uppercase font-bold tracking-wider">
              <tr>
                ...
                <th className="px-6 py-4">Prices (Sale / Purchase)</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredProducts.length > 0 ? (
                filteredProducts.map((product) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-sm text-gray-600">
                      <span className="px-2 py-0.5 rounded-lg bg-gray-100 text-[10px] font-bold uppercase tracking-tight">
                        {product.category}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    {role === UserRole.ADMIN && (
                      <td className="px-6 py-4 text-right" onClick={(e) => e.stopPropagation()}>
                        <button onClick={() => onDeleteProduct(product.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg">
                          <Trash2 size={16} />
                        </button>
                      </td>
                    )}
                ...
                <tr><td colSpan={4} className="py-20 text-center text-gray-400">No products found</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="fixed inset-0 z-[110] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white w-full max-w-lg rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden animate-slideUp">
            <div className="p-6 border-b border-gray-100 flex items-center justify-between">
              <h3 className="text-lg font-black text-gray-900">New Product</h3>
            ...
            <form onSubmit={handleAddSubmit} className="p-6 space-y-5 max-h-[80vh] overflow-y-auto">
              <div className="flex flex-col items-center gap-2">
                ...
                >
                  {formData.image ? <img src={formData.image} className="w-full h-full object-cover" alt="Preview" /> : <Camera className="text-gray-300" size={32} />}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2 space-y-1">
                  ...
                  <input required className="w-full px-4 py-3 bg-gray-50 border-gray-100 rounded-xl outline-none text-sm" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                </div>
              ...
              <button type="submit" className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-bold transition-all shadow-xl">Add Product</button>
            </form>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn" onClick={() => setSelectedProduct(null)}>
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="relative h-48 bg-gray-100 flex items-center justify-center overflow-hidden">
              {selectedProduct.image ? <img src={selectedProduct.image} className="w-full h-full object-cover" alt={selectedProduct.name} /> : <Package className="text-gray-200" size={80} />}
              <button onClick={() => setSelectedProduct(null)} className="absolute top-4 right-4 p-2 bg-white/90 rounded-full shadow-lg"><X size={18} /></button>
            </div>
            <div className="p-8">
              <span className="text-[10px] font-black uppercase text-indigo-500 bg-indigo-50 px-2 py-1 rounded-lg">{selectedProduct.category}</span>
              <h3 className="text-2xl font-black text-gray-900 mt-2">{selectedProduct.name}</h3>
              <p className="text-sm text-gray-500 mt-4">{selectedProduct.description || "No description provided."}</p>
              <div className="grid grid-cols-2 gap-4 mt-8">
                <div className="p-4 bg-emerald-50 rounded-2xl border border-emerald-100">
                  <p className="text-[10px] font-bold text-emerald-600 uppercase">Sale Price</p>
                  <p className="text-2xl font-black text-emerald-700">${selectedProduct.salePrice}</p>
                </div>
                <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100">
                  <p className="text-[10px] font-bold text-gray-500 uppercase">Cost</p>
